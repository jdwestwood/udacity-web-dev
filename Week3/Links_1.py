...
# make the function queryID() return the number of votes for the link whose ID is
# 15
...
# make the function queryUser() return the number of votes for the link whose
# submitter_id is 62443

def queryUser(userNo):
    linksList = []
    for link in links:
        if link.submitter_id == userNo:
            linksList.append(link)
    linksList.sort(key=lambda link: link.submitted_time)
    return linksList

# Exercise 1
findID = 15
print queryID(findID)

# Exercise 2
userNo = 62443
print queryUser(userNo)
...
for l in links:                         # populate the database
...
def querySQL(queryString):
    cursor = db.execute(queryString)

#    link = Link(*cursor.fetchone())  # note the use of * to convert from a tuple to a
#    return link.votes                # list of arguments for the Link constructor to
#    return link.id                   # create the namedtuple defined by Link above!
    idList = []
    for result_tuple in cursor:
        idList.append(*result_tuple)
# or as a list comprehension:
#   idList = [result_tuple[0] for result_tuple in cursor]
    return idList

query1 = "select * from links where id=2"
query2 = "select * from links where submitter_id=62443 and votes>1000"
query3 = "select id from links where submitter_id=62443 order by submitted_time asc"  # or desc
#print querySQL(query1)
#print querySQL(query2)
print querySQL(query3)

# QUIZ - implement the function link_by_id() that takes a link's ID and returns
# the link itself

def build_link_index():
    return dict(zip([link.id for link in links],links))

linksDict = build_link_index()

def link_by_id(link_id):
    global linksDict
    return linksDict.get(link_id)   # returns None if link_id does not exists
